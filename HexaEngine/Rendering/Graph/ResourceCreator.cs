namespace HexaEngine.Rendering.Graph
{
    ...
    public class ResourceCreator
    {
        ...
        public ResourceCreator(IGraphicsDevice device)
        {
            this.device = device;
        }
        ...
        private readonly List<DepthStencilBufferDesc> depthMipChainDescriptions = new();
        private readonly List<DepthMipChain> depthMipChains = new();
        private readonly Dictionary<string, int> nameToDepthMipChain = new();

        private readonly List<DepthStencilBufferDesc> depthStencilBufferDescriptions = new();
        ...
        internal void CreateResources()
        {
            ...
            {
                depthMipChains.Add(new(device, depthMipChainDescriptions[i]));
            }

            for (int i = 0; i < depthStencilBufferDescriptions.Count; i++)
            {
                depthStencilBuffers.Add(new(device, depthStencilBufferDescriptions[i]));
            }

            for (int i = 0; i < gBufferDescriptions.Count; i++)
            {
                gBuffers.Add(new(device, gBufferDescriptions[i]));
            }

            for (int i = 0; i < texture1DDescriptions.Count; i++)
            {
                textures1d.Add(new(device, texture1DDescriptions[i]));
            }

            for (int i = 0; i < texture2DDescriptions.Count; i++)
            {
                textures2d.Add(new(device, texture2DDescriptions[i]));
            }

            for (int i = 0; i < texture3DDescriptions.Count; i++)
            {
                textures3d.Add(new(device, texture3DDescriptions[i]));
            }
        }
        ...
        public int CreateDepthMipChain(string name, DepthStencilBufferDesc description)
        {
        ...
        public int CreateDepthStencilBuffer(string name, DepthStencilBufferDesc description)
        {
        ...
        public ISamplerState CreateSamplerState(string name, SamplerStateDescription description)
        {
            ...
            {
                if (samplerStateDescriptions[samplerStateIndex] == description)
                {
