namespace HexaEngine.Editor.ModelEditor
{
    ...
    public class ModelNodeProperties : EditorWindow
    {
        private readonly ModelEditorWindow editor;
        private readonly ModelNodes nodes;

        public ModelNodeProperties(ModelEditorWindow editor, ModelNodes nodes)
        {
            IsShown = true;
            this.editor = editor;
            this.nodes = nodes;
        ...
        public static unsafe bool TransformEdit(string label, ref Vector3 translation, ref Quaternion rotation, ref Vector3 scale)
        {
            ...
            if (child_visible)
            {
                ImGui.Text($"<Pos: {translation}, Rot: {rotation.GetRotation().ToDeg()}, Scale: {scale}>");
                ...
                if (ImGui.BeginPopup("TransformEdit"))
                {
                    ...
                    {
                        var val = rotation.GetRotation().ToDeg();
