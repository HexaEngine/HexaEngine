namespace HexaEngine.Editor.Properties.Editors
{
    ...
    internal class TerrainEditor : IObjectEditor
    {
        ...
        public Type Type => typeof(TerrainRenderer);
        ...
        {
            if (Instance is not TerrainRenderer component)
            {
                return;
            }
            ...
            for (int i = 0; i < grid.Count; i++)
            {
                ...
                if (editTerrain || editMask)
                {
                    ...
                    context.PSSetConstantBuffer(brushBuffer, 0);
                    context.PSSetConstantBuffer(camera.Value, 1);
                    if (editMask)
                    {
                        context.PSSetShaderResource(mask.SRV, 0);
                        context.PSSetSampler(maskSampler, 0);
