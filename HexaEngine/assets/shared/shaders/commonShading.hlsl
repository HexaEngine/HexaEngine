...
float3 DirectionalLightBRDF(Light light, PixelParams pixel)
{
    ...
    if (castsShadows)
    {
        ...
        visibility = ShadowFactorDirectionalLightCascaded(shadowSampler, depthCSM, data, pixel.Pos, clight.NdotL);
    }
...
float3 PointLightBRDF(Light light, PixelParams pixel)
{
    ...
    {
        ShadowData data = shadowData[light.shadowMapIndex];
        visibility = ShadowFactorPointLight(shadowSampler, depthAtlas, light, data, pixel.Pos, clight.NdotL);
    }
...
float3 SpotlightBRDF(Light light, PixelParams pixel)
{
    ...
    if (theta > light.outerCosine)
    {
        ...
        {
            ShadowData data = shadowData[light.shadowMapIndex];
            visibility = ShadowFactorSpotlight(shadowSampler, depthAtlas, data, pixel.Pos, clight.NdotL);
        }
