...
{
    float2 pos = (screen_space_position.xy - 0.5) * float2(2, -2);
    ...
    point VertexOut p[1], inout TriangleStream<GeometryOut> triStream)
{
    ...
    float referenceDepth = saturate(screen_space_position.z);
    ...
    for (float y = -range.y; y <= range.y; y += step.y)
    {
        ...
        {
            samples += 1.0f;
            accdepth += depth_texture.SampleLevel(point_clamp_sampler, screen_space_position.xy + float2(x, y), 0).r >= referenceDepth - 0.001 ? 1 : 0;
        }
    }
    accdepth /= samples;
