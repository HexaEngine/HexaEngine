...
cbuffer LUTParams
{
    float LUT_AmountChroma;
    float LUT_AmountLuma;
};
...
void main(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 res : SV_Target0)
{
    ...
    color.xyz = lerp(normalize(color.xyz), normalize(lutcolor.xyz), LUT_AmountChroma) * lerp(length(color.xyz), length(lutcolor.xyz), LUT_AmountLuma);
