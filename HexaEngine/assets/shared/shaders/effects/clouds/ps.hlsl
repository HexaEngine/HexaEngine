...
Output main(VertexOut pin)
{
    ...
    float4 rayView = mul(rayClipSpace, projInv);
    rayView = float4(rayView.xy, 1.0, 0.0);

    float3 worldDir = mul(rayView, viewInv).xyz;
    worldDir = normalize(worldDir);

    float3 startPos, endPos;

    bool intersect = IntersectSphere(GetCameraPos().xyz, worldDir, startPos, endPos);
