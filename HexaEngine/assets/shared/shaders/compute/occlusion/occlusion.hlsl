...
RWStructuredBuffer<uint> instanceOffsets : register(u2);
RWStructuredBuffer<DrawIndexedInstancedIndirectArgs> drawArgs : register(u3);
...
void main(uint3 threadID : SV_DispatchThreadID)
{
	...
	{
		float3 bboxMin = instanceDataIn[threadID.x].min.xyz;
		float3 bboxMax = instanceDataIn[threadID.x].max.xyz;
		...
		float4 depth = float4(inputRT.SampleLevel(samplerPoint, boxUVs.xy, mip).r,
		...
		if (!ActivateCulling || minZ <= maxDepth)
		{
			predicate = 1;
			instanceDataOut[threadID.x] = instanceDataIn[threadID.x].world;

			//increase instance count for this particular prop type
			InterlockedAdd(instanceCounts[instanceDataIn[threadID.x].type], 1);
		}

		temp[threadID.x] = predicate;
		...
		if (threadID.x == 0)
		{
			...
			uint baseOffset = 0;
			for (uint i = 0; i < NoofPropTypes; i++)
			{
				instanceOffsets[i] = baseOffset;
				drawArgs[i].InstanceCount = instanceCounts[i];
				baseOffset += instanceCounts[i];
			}
		}
	}
}