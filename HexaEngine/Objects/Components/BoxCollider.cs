    ...
    public class BoxCollider : IComponent
    {
        ...
        private void Init()
        {
            ...
            if (Type == ColliderType.Dynamic)
            {
                ...
                bodyHandle = scene.Simulation.Bodies.Add(BodyDescription.CreateDynamic(pose, new BodyVelocity(), inertia, new CollidableDescription(index), new(1)));
                bodyReference = scene.Simulation.Bodies.GetBodyReference(bodyHandle);
            }
            if (Type == ColliderType.Kinematic)
            {
                bodyHandle = scene.Simulation.Bodies.Add(BodyDescription.CreateKinematic(pose, new BodyVelocity(), new CollidableDescription(index), new(1)));
                bodyReference = scene.Simulation.Bodies.GetBodyReference(bodyHandle);
            }
        }
