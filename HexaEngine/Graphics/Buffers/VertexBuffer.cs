namespace HexaEngine.Graphics.Buffers
{
    ...
    public unsafe class VertexBuffer : IBuffer
    {
        ...
        public VertexBuffer(IGraphicsDevice device, CpuAccessFlags flags, [CallerFilePath] string name = "")
        {
            ...
            items = Alloc<Vertex>(DefaultCapacity);
            ZeroRange(items, DefaultCapacity);
            capacity = DefaultCapacity;

            description = new(sizeof(Vertex) * DefaultCapacity, BindFlags.VertexBuffer, Usage.Default, flags);
        ...
        public VertexBuffer(IGraphicsDevice device, CpuAccessFlags flags, Vertex[] vertices, [CallerFilePath] string name = "")
        {
            this.device = device;
            this.name = name;

            capacity = (uint)vertices.Length;
            items = Alloc<Vertex>(capacity);
            count = capacity;
            ...
            description = new(sizeof(Vertex) * (int)capacity, BindFlags.VertexBuffer, Usage.Default, flags);
            if (flags.HasFlag(CpuAccessFlags.Write))
            {
                description.Usage = Usage.Dynamic;
            }
        ...
        public uint Stride { get; } = (uint)sizeof(Vertex);
        ...
        public uint Capacity
        {
            ...
            set
            {
                var tmp = Alloc<Vertex>((int)value);
                var oldsize = count * sizeof(Vertex);
                var newsize = value * sizeof(Vertex);
        ...
        {
            var size = (count - index) * sizeof(Vertex);
        ...
        public bool Update(IGraphicsContext context)
        {
            if (isDirty)
            {
                context.Write(buffer, items, (int)(count * sizeof(Vertex)));
                isDirty = false;
                return true;
            }
            return false;
