...
{
    HRESULT result = img->Initialize(mdata, flags);
    return result;
}
...
void *GetBufferPointer(DirectX::Blob *blob) noexcept
{
    return blob->GetBufferPointer();
}
size_t GetBufferSize(DirectX::Blob *blob) noexcept
{
    return blob->GetBufferSize();
}

HRESULT Resize(DirectX::Blob *blob, size_t size) noexcept
{
    return blob->Resize(size);
}
// Reallocate for a new size

HRESULT Trim(DirectX::Blob *blob, size_t size) noexcept
{
