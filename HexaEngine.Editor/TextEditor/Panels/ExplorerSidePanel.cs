namespace HexaEngine.Editor.TextEditor.Panels
{
    ...
    public unsafe class ExplorerSidePanel : SidePanel
    {
        ...
        {
            ProjectManager.ProjectLoaded += ProjectManagerProjectLoaded;
            fileSystemState.Refresh();
        }

        private void ProjectManagerProjectLoaded(HexaProject project)
        {
            var folder = ProjectManager.CurrentProjectFolder;
            if (folder == null)
            {
                return;
            }
        ...
        public unsafe class FileSystemState
        {
            ...
            private static void ProcessDirectory(string currentDirectory, int parentIndex, FileSystemItem* self, UnsafeList<FileSystemItem>* items, int depth)
            {
                ...
                {
                    if (SourceAssetsDatabase.IsIgnored(directory))
                        continue;
                ...
                {
                    if (SourceAssetsDatabase.IsIgnored(file))
                        continue;
                    int newItemIndex = items->Count;
                    items->Add(new FileSystemItem
                    {
